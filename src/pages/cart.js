...
export default function Cart() {
  ...
  let rows = items?.map((i, index) => {
    return {
      _id: i._id,
      image: `/images/products/p${index + 1}.png`,
      name: i.name,
      category: i.category,
      quantity: i.quantity,
  ...
  const heads = useMemo(() => ["Image", "Name", "Category", "Quantity", "Price", ""], []);
  useEffect(() => {
      ...
      {items?.length == 0 ? (
        ...
        <Box>
          ...
          >
            ...
            <Typography as="h1" className={style.cartTitle} sx={{ color: "primary.main", mt: "4px" }}>
              ${totalPrice}
            </Typography>
          </Box>

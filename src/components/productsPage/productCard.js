...
export const ProductCard = ({ product }) => {
  ...
  return (
    <Box>
      {success && (
        <AlertMessage msg='Adding a new item to the cart successfully' />
      )}
      <Box className={style.card}>
        ...
        <Image src={product?.image} alt='product' width={220} height={220} />
        ...
        <Box className={style.productIcons}>
          {items?.findIndex((i) => i._id == product._id) > -1 ? (
            ...
            >
              ...
              >
                <CheckIcon
                  sx={{ color: 'inherit', width: '20px', height: '20px' }}
                />
              </Box>
            </IconButton>
          ) : (

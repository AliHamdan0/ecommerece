import { Box, Menu, MenuItem, Button, Typography, Divider } from "@mui/material";
...
export const HeaderCartMenu = ({ anchorEl, setAnchorEl, toggleDrawer = () => "" }) => {
  ...
  useEffect(() => {
    const calculatePrice = () => {
      let sum = 0;
      for (let i = 0; i < items?.length; i++) sum += items[i].price * items[i].quantity;
      return sum;
    };
  ...
  return (
    ...
    >
      {items?.length == 0 ? (
        <Typography component="h6" className={style.cartFont} sx={{ textAlign: "center", my: 2 }}>
          Your cart is empty
        </Typography>
      ) : (
        <Box>
          {items?.map((item, index) => (
            <Fragment key={item._id}>
              {/* <MenuItem> */}
              <Box sx={{ p: 2, my: 0 }}>
                <ShoppingItem item={item} index={index} />
              </Box>
              {/* </MenuItem> */}
              <Divider />
            </Fragment>
          ...
          <MenuItem>
            <Box sx={{ display: "flex", mx: "auto", mt: 2 }}>
              ...
              <Typography component="h6" className={style.cartFont} sx={{ color: "primary.main", mx: 1 }}>
                ${totalPrice}
              </Typography>
            </Box>
          </MenuItem>
